...
from src.util.balances import get_usdt_balance, convert_usdt_to_btc, synchronize_binance_time
from src.util.nums import truncate_to_first_significant_digit, cont_zero, create_num_witg_zero
...
usdt_balance = get_usdt_balance(client_binance=client_binance,active_operated=purchase_currency)
print(f" {purchase_currency}: {usdt_balance}")
qaunt_active = convert_usdt_to_btc(usdt_balance=usdt_balance, client_binance=client_binance,symbol=active_code_)
print(qaunt_active)
...
def get_data(codigo, intervalo):
    ...
    price.columns = ["tempo_abertura", "abertura", "maxima", "minima", "fechamento", "volume", "tempo_fechamento", "moedas_negociadas", "numero_trades",
                    "volume_ativo_base_compra", "volume_ativo_cotação", "-"]
    price = price[["fechamento", "tempo_fechamento"]]
    price["tempo_fechamento"] = pd.to_datetime(price["tempo_fechamento"], unit = "ms").dt.tz_localize("UTC")
    price["tempo_fechamento"] = price["tempo_fechamento"].dt.tz_convert("America/Sao_Paulo")
...
def commercial_strategy(data, active_code, active_operated, quant, position):
    ...
    print(f"Última Média Rápida: {fast_media_slow} | Última Média Devagar: {last_media_slow}")
    ...
    if fast_media_slow > last_media_slow:
        ...
        if position == False:
            ...
            order = client_binance.create_order(symbol = active_code,
                side = SIDE_BUY,
        ...
        if position == True:
            q = create_num_witg_zero(cont_zero(qaunt_active))
            order = client_binance.create_order(symbol = active_code,
                side = SIDE_SELL,
                type = ORDER_TYPE_MARKET,
...
while True:
    ...
    current_position = commercial_strategy(dados_atualizados, active_code=active_code_,
                                        active_operated=active_operated_, quant=qaunt_active, position=current_position)
